-- Extend lead→contact conversion to copy health-insurance coverage fields and
-- resolve start date from health_insurance_start_date (not just start_date).
...
BEGIN
  ...
  IF p_merge_into_contact_id IS NOT NULL THEN
    ...
    UPDATE crm_records
    SET data = (
      SELECT COALESCE(jsonb_object_agg(COALESCE(ek.key, nk.key), COALESCE(ek.value, nk.value)), '{}'::jsonb)
  ...
  ELSE
    ...
    INSERT INTO crm_records (
      ...
      NULLIF(v_lead_data->>'health_insurance_carrier', '')::uuid
    )
