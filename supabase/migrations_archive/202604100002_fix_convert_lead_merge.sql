-- Fix: Lead-to-Contact conversion fails because API passes p_merge_into_contact_id
-- but the RPC only accepts 2 parameters. This replaces the function with a 3-param
-- version that supports optional merge-into-existing-contact logic.

-- Drop old 2-param overload so there is no ambiguity
DROP FUNCTION IF EXISTS public.convert_lead_to_contact(uuid, uuid);
...
BEGIN
  ...
  -- 5. Build contact data JSONB with field mapping
  ...
  -- 7. Merge or create: if a merge target was specified, update that contact
  IF p_merge_into_contact_id IS NOT NULL THEN
    -- Verify the target contact exists in the same org
    ...
    -- Merge: lead data fills in blanks on the existing contact (does not overwrite)
    UPDATE crm_records
    SET data = (
  ...
  ELSE
    -- No merge target: check for duplicate email first
  ...
  -- 8. Move all crm_notes from lead to the contact so notes are never orphaned
  UPDATE crm_notes
  SET record_id = v_new_contact_id
  WHERE record_id = p_lead_record_id;

  -- Also move any general notes linked to the lead
  UPDATE notes
  SET record_id = v_new_contact_id::text
  WHERE record_id = p_lead_record_id::text;

  -- 10. Create record link: lead → contact
  ...
  -- 11. Update the lead record: mark as converted
  UPDATE crm_records
