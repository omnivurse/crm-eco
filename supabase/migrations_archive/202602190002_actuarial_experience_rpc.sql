-- =============================================================================
-- Actuarial Experience Data RPC (De-Identified / Aggregated Only)
--
-- Returns population-level financial experience data for actuarial analysis:
--   1. Monthly Experience Summary (exposure, contributions, needs, loss ratio)
--   2. Needs Distribution by Type (utilization mix)
--   3. Contribution Adequacy (revenue side)
--   4. Age-Band Loss Experience (risk segmentation)
--
-- Zero PHI. Zero individual records. Only aggregated financial behavior.
-- Small-group suppression: age-band rows with < 5 members return 0.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.get_actuarial_experience(
  p_org_id uuid,
...
BEGIN
  ...
  v_start_date := DATE_TRUNC('month', CURRENT_DATE) - (p_months - 1) * INTERVAL '1 month';

  SELECT jsonb_build_object(
    'monthlyExperience',     (SELECT public._actuarial_monthly(p_org_id, v_start_date)),
    'needsByType',           (SELECT public._actuarial_needs_by_type(p_org_id, v_start_date)),
    'contributionAdequacy',  (SELECT public._actuarial_contributions(p_org_id, v_start_date)),
    'ageBandExperience',     (SELECT public._actuarial_age_bands(p_org_id, v_start_date)),
    'exposureDevelopment',   (SELECT public._actuarial_exposure_dev(p_org_id, v_start_date)),
    'summary',               (SELECT public._actuarial_summary(p_org_id, v_start_date)),
    'periodMonths', p_months,
    'generatedAt', now()
  ) INTO v_result;
...
GRANT EXECUTE ON FUNCTION public.get_actuarial_experience(uuid, int) TO authenticated;
  ...
  RETURN (
    ...
    monthly_needs AS (
      SELECT
        ...
        ROUND(COALESCE(AVG(n.total_amount), 0), 2)       AS avg_need,
        ROUND(COALESCE(
    ...
    CROSS JOIN LATERAL (
      SELECT jsonb_build_object(
        ...
        'lossRatio',                 CASE WHEN COALESCE(mr.collected, 0) > 0
          THEN ROUND((COALESCE(mn.paid_amount, 0) / mr.collected) * 100, 1)
          ELSE 0 END,
        'avgMemberContribution',     CASE WHEN COALESCE(mm.eligible, 0) > 0
          THEN ROUND(COALESCE(mr.collected, 0) / mm.eligible, 2)
          ELSE 0 END,
  ...
  RETURN (
    WITH type_agg AS (
      SELECT
        ...
        ROUND(COALESCE(AVG(n.total_amount), 0), 2)       AS avg_amount
      FROM needs n
      WHERE n.organization_id = p_org_id
        AND n.created_at >= p_start
      GROUP BY 1
  ...
  RETURN (
    ...
    monthly_expected AS (
      SELECT
        ...
        ROUND(COALESCE(AVG(bs.amount), 0), 2) AS avg_contribution,
        ROUND(COALESCE(
          PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY bs.amount), 0
        ), 2) AS median_contribution
      FROM months m
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'collectionRate',      CASE WHEN me.expected > 0
        THEN ROUND((COALESCE(ma.collected, 0) / me.expected) * 100, 1)
        ELSE 0 END
...
-- =============================================================================
-- 5. Portfolio Summary (lifetime + trailing 12-month KPIs)
-- =============================================================================
...
BEGIN
  ...
  v_loss_ratio := CASE WHEN v_total_collected > 0
    THEN ROUND((v_total_paid / v_total_collected) * 100, 1)
    ELSE 0 END;

  v_collection_rate := CASE WHEN v_total_expected > 0
    THEN ROUND((v_total_collected / v_total_expected) * 100, 1)
    ELSE 0 END;

  v_utilization_rate := CASE WHEN v_active_members > 0
    THEN ROUND((v_total_needs::numeric / v_active_members) * 100, 1)
    ELSE 0 END;
...
-- =============================================================================
-- 6. Exposure & Development (Member-Months, IBNR, Lag Analysis)
--
-- Member-months are calculated as true daily exposure: the count of members
-- eligible for sharing on each day of the month, summed and divided by the
-- number of days in the month.
--
-- Incurred amounts are attributed to the date of service (incident_date),
-- NOT the date of submission or approval, enabling actuarial loss
-- development / completion triangle analysis.
-- =============================================================================
...
BEGIN
  RETURN (
    ...
    -- Daily exposure: count eligible members per day, then average for member-months
    daily_exposure AS (
      SELECT
    ...
    -- Claims attributed to SERVICE MONTH (incident_date), not submission month
    service_month_claims AS (
      SELECT
        ...
        ) FILTER (WHERE n.incident_date IS NOT NULL), 0)) AS avg_report_lag,
        ROUND(COALESCE(AVG(
          EXTRACT(DAY FROM (
            COALESCE(n.payment_date::timestamptz, n.last_status_change_at) - n.created_at
          ))
        ) FILTER (WHERE n.reimbursed_amount > 0), 0)) AS avg_payment_lag
      FROM needs n
      WHERE n.organization_id = p_org_id
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'pmpm',              CASE
        ...
        THEN ROUND(COALESCE(sc.incurred_amount, 0) / de.member_months, 2)
        ELSE 0 END
