-- Phase 3: Supabase linter — revoke EXECUTE on internal SECURITY DEFINER functions
-- Linter: anon/authenticated_security_definer_function_executable
-- Strategy: revoke API access to trigger/internal/cron helpers only.
-- Keeps 167 legitimate RPC + RLS helper functions callable by authenticated.
-- Does NOT revoke service_role (edge functions, cron routes use service key).
