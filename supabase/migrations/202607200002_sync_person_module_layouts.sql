-- Sync person-module layouts with live crm_fields sections:
--   1) Create Default Member Layout when members has fields but no layout
--   2) Add any field-backed section missing from default contacts/leads/members layouts
-- Idempotent. Does not remove existing sections. Coverage empty bands left intact.
...
BEGIN
  ...
  LOOP
    ...
    -- Clear prior iteration (SELECT INTO without rows keeps old values).
    v_layout_id := NULL;
    v_config := NULL;
    ...
    -- Members (or any person module) with no default layout: create one.
    IF v_layout_id IS NULL THEN
      ...
      -- Any field sections not in canonical order — append alphabetically
    ...
    -- Backup once, then merge missing field-backed sections into existing default.
    INSERT INTO public.crm_layout_section_sync_backup_20260720 (layout_id, config_before)
    ...
    -- Stable sort by canonical order, unknowns last (alpha)
