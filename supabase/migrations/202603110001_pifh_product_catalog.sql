-- ============================================================================
-- PHASE A + B + C: PIFH Enterprise Product Catalog
...
COMMENT ON COLUMN plans.plan_code IS 'Unique plan identifier (e.g. PIF-ESS-2026)';
COMMENT ON COLUMN plans.brand_name IS 'Public-facing plan name (e.g. PIF Essentials)';
...
-- ============================================================================
-- B.2  Seed PIF Health Share Plans
-- ============================================================================
DO $$
DECLARE
  v_org_id uuid;
BEGIN
  ...
  VALUES (v_org_id, 'PIF Essentials', 'PIF-ESS-2025', 'PIF-ESS-2025', 'PIF Essentials', 'pif_essentials', 'healthshare', 2025, true, true,
    'Affordable health sharing for individuals and families. Basic coverage with telehealth and preventive care.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Direct
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Direct', 'PIF-DIR-2025', 'PIF-DIR-2025', 'PIF Direct', 'pif_direct', 'healthshare', 2025, true, true,
    'Direct primary care sharing with enhanced doctor access and lower share amounts.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Care Plus
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Care Plus', 'PIF-CP-2025', 'PIF-CP-2025', 'PIF Care Plus', 'pif_care_plus', 'healthshare', 2025, true, true,
    'Comprehensive sharing with maternity, mental health, and expanded provider network.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Secure HSA
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Secure HSA', 'PIF-SHSA-2025', 'PIF-SHSA-2025', 'PIF Secure HSA', 'pif_secure_hsa', 'healthshare', 2025, true, true,
    'HSA-compatible health sharing with higher annual unshared amounts and tax-advantaged savings.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Premium Care
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Premium Care', 'PIF-PREM-2025', 'PIF-PREM-2025', 'PIF Premium Care', 'pif_premium_care', 'healthshare', 2025, true, true,
    'Premium-tier sharing with lowest unshared amounts, full maternity, and concierge support.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Premium HSA
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Premium HSA', 'PIF-PHSA-2025', 'PIF-PHSA-2025', 'PIF Premium HSA', 'pif_premium_hsa', 'healthshare', 2025, true, true,
  ...
  FROM plans p WHERE p.code = 'PIF-ESS-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);

  INSERT INTO plan_healthshare_config (plan_id, individual_share_amount, family_share_amount, annual_unshared_amount, max_sharing_limit, telehealth_included, maternity_included, rx_discount_included, preventive_included, network_type, waiting_period_days, pre_existing_waiting_days)
  SELECT p.id, 200, 400, 3500, 1000000, true, false, true, true, 'PPO', 30, 365
  FROM plans p WHERE p.code = 'PIF-DIR-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);
  ...
  FROM plans p WHERE p.code = 'PIF-CP-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);

  INSERT INTO plan_healthshare_config (plan_id, individual_share_amount, family_share_amount, annual_unshared_amount, max_sharing_limit, telehealth_included, maternity_included, rx_discount_included, preventive_included, network_type, waiting_period_days, pre_existing_waiting_days)
  SELECT p.id, 275, 550, 6350, 1000000, true, false, true, true, 'open', 30, 365
  FROM plans p WHERE p.code = 'PIF-SHSA-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);
  ...
  FROM plans p WHERE p.code = 'PIF-PREM-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);

  INSERT INTO plan_healthshare_config (plan_id, individual_share_amount, family_share_amount, annual_unshared_amount, max_sharing_limit, telehealth_included, maternity_included, rx_discount_included, preventive_included, mental_health_included, vision_included, dental_included, network_type, waiting_period_days, pre_existing_waiting_days)
  SELECT p.id, 375, 750, 7000, 2000000, true, true, true, true, true, true, true, 'PPO', 0, 90
  FROM plans p WHERE p.code = 'PIF-PHSA-2025' AND NOT EXISTS (SELECT 1 FROM plan_healthshare_config WHERE plan_id = p.id);
...
-- ============================================================================
-- C.2  Seed PIF Traditional Insurance Plans
-- ============================================================================
DO $$
DECLARE
  v_org_id uuid;
BEGIN
  ...
  VALUES (v_org_id, 'PIF Bronze 5000', 'PIF-BRZ-5000', 'PIF-BRZ-5000', 'PIF Bronze 5000', 'pif_bronze_5000', 'traditional', 'bronze', 2025, true, true,
    'High-deductible bronze plan with lower monthly premiums. HSA eligible.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Silver 4000
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, metal_tier, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Silver 4000', 'PIF-SLV-4000', 'PIF-SLV-4000', 'PIF Silver 4000', 'pif_silver_4000', 'traditional', 'silver', 2025, true, true,
    'Balanced silver plan with moderate deductible and broad network coverage.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Gold 2000
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, metal_tier, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Gold 2000', 'PIF-GLD-2000', 'PIF-GLD-2000', 'PIF Gold 2000', 'pif_gold_2000', 'traditional', 'gold', 2025, true, true,
    'Lower deductible gold plan with enhanced benefits and predictable copays.')
  ON CONFLICT (organization_id, code) DO NOTHING;

  -- PIF Platinum 0
  INSERT INTO plans (organization_id, name, code, plan_code, brand_name, internal_name, category, metal_tier, effective_year, is_active, public_visible, description)
  VALUES (v_org_id, 'PIF Platinum 0', 'PIF-PLT-0', 'PIF-PLT-0', 'PIF Platinum 0', 'pif_platinum_0', 'traditional', 'platinum', 2025, true, true,
  ...
  FROM plans p WHERE p.code = 'PIF-BRZ-5000' AND NOT EXISTS (SELECT 1 FROM plan_traditional_config WHERE plan_id = p.id);

  INSERT INTO plan_traditional_config (plan_id, deductible_individual, deductible_family, oop_max_individual, oop_max_family, coinsurance_percent, primary_care_copay, specialist_copay, er_copay, urgent_care_copay, rx_generic_copay, rx_brand_copay, rx_specialty_copay, telehealth_copay, mental_health_copay, hsa_eligible, network_type)
  SELECT p.id, 4000, 8000, 7900, 15800, 30, 30, 60, 300, 50, 10, 40, 200, 0, 30, false, 'PPO'
  FROM plans p WHERE p.code = 'PIF-SLV-4000' AND NOT EXISTS (SELECT 1 FROM plan_traditional_config WHERE plan_id = p.id);

  INSERT INTO plan_traditional_config (plan_id, deductible_individual, deductible_family, oop_max_individual, oop_max_family, coinsurance_percent, primary_care_copay, specialist_copay, er_copay, urgent_care_copay, rx_generic_copay, rx_brand_copay, rx_specialty_copay, telehealth_copay, mental_health_copay, hsa_eligible, network_type)
  SELECT p.id, 2000, 4000, 6500, 13000, 20, 20, 40, 250, 35, 10, 35, 150, 0, 25, false, 'PPO'
  FROM plans p WHERE p.code = 'PIF-GLD-2000' AND NOT EXISTS (SELECT 1 FROM plan_traditional_config WHERE plan_id = p.id);

  INSERT INTO plan_traditional_config (plan_id, deductible_individual, deductible_family, oop_max_individual, oop_max_family, coinsurance_percent, primary_care_copay, specialist_copay, er_copay, urgent_care_copay, rx_generic_copay, rx_brand_copay, rx_specialty_copay, telehealth_copay, mental_health_copay, hsa_eligible, network_type)
  SELECT p.id, 0, 0, 4000, 8000, 10, 10, 25, 150, 25, 5, 25, 100, 0, 15, false, 'EPO'
  FROM plans p WHERE p.code = 'PIF-PLT-0' AND NOT EXISTS (SELECT 1 FROM plan_traditional_config WHERE plan_id = p.id);
