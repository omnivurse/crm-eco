-- Undo-delete Phase 1 (read-hiding): keep soft-deleted (trashed) crm_records out
-- of global search. Signature-preserving CREATE OR REPLACE of the two search RPCs
-- from 202607140001 — the ONLY change is `AND r.deleted_at IS NULL` added to the
-- final SELECT's WHERE in each, so trashed rows never surface in the ⌘K palette,
-- the module spotlight, or the search page (which would otherwise let a user
-- click through to a trashed record's detail).
