/*
  # My Work - Integrations, OAuth Tokens, and Password Vault References

  1. New Tables
    - `integrations`
      - `id` (uuid, primary key)
      - `user_id` (uuid, references profiles)
      - `provider` (enum: ms_teams, google, onepassword, bitwarden, vault)
...
CREATE TABLE IF NOT EXISTS public.integrations (
  ...
  provider text NOT NULL CHECK (provider IN ('ms_teams', 'google', 'onepassword', 'bitwarden', 'vault')),
...
CREATE TABLE IF NOT EXISTS public.oauth_tokens (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  provider text NOT NULL CHECK (provider IN ('ms_teams', 'google')),
