-- Batched version of upsert that processes a range of rows to avoid gateway timeout
-- Called multiple times from the TypeScript script with different offsets
...
BEGIN
  ...
  LOOP
    BEGIN
      SELECT id INTO v_parent_record_id FROM crm_records
      WHERE org_id = v_org_id AND data->>'zoho_record_id' = v_row.parent_id
      LIMIT 1;
      ...
      VALUES (v_org_id, v_parent_record_id, v_row.note_content, false,
              COALESCE(_parse_import_datetime(v_row.created_time), now()),
              COALESCE(_parse_import_datetime(v_row.modified_time), now()));
