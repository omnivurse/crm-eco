...
-- Additive / reversible. PROD WRITE RISK: YES when applied (DDL + seed rows).
-- Do not apply to production without explicit approval.
--
-- Adds:
--   - Seed keys: payables.read|create|approve|pay
--   - _org_role_implies_permission (legacy membership bridge)
--   - has_org_permission(org_id, key)
--   - list_org_permissions(org_id) → text[]
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- 2) Legacy org-role bridge (temporary compatibility)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Owners / super_admins / admins: full access during bridge period
...
-- ---------------------------------------------------------------------------
-- 3) has_org_permission — org-scoped check (preferred over has_crm_permission)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Super-admin short-circuit (mirrors has_crm_role)
  ...
  -- Catalog path: crm_user_roles → roles (org-scoped or global) → permissions
  IF EXISTS (
    SELECT 1
...
-- ---------------------------------------------------------------------------
-- 4) list_org_permissions — bootstrap client permission set
-- ---------------------------------------------------------------------------
...
-- Bridge helper is internal; do not grant to authenticated (called only by has_org_permission)

-- ---------------------------------------------------------------------------
-- Rollback notes (manual):
--   DROP FUNCTION IF EXISTS public.list_org_permissions(uuid);
--   DROP FUNCTION IF EXISTS public.has_org_permission(uuid, text);
--   DROP FUNCTION IF EXISTS public._org_role_implies_permission(uuid, text);
--   DELETE FROM public.crm_permissions
--     WHERE key IN ('payables.read','payables.create','payables.approve','payables.pay');
-- =============================================================================
