...
export function AuthHeroPanel({
  ...
  const accentDot =
    variant === 'admin'
      ? 'bg-emerald-400 shadow-[0_0_0_3px_rgba(5,150,105,0.25)]'
      : 'bg-cyan-400 shadow-[0_0_0_3px_rgba(6,182,212,0.2)]';
  ...
  return (
    <div className="relative flex h-full w-full flex-col justify-center">
      ...
      <div className="relative z-10 px-12 xl:px-16">
        {badge && (
          <span className="mb-8 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/70">
            <span className={`h-1.5 w-1.5 rounded-full ${accentDot}`} />
            {badge}
          </span>
        )}
        <h1 className="mb-5 font-heading text-5xl font-bold leading-[1.05] tracking-[-0.04em] text-white xl:text-6xl">
          {headline}
        </h1>
        <p className="mb-10 max-w-md text-lg leading-relaxed text-white/55">{subtitle}</p>
        {children}
