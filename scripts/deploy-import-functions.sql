...
-- ============================================================================
-- PART 3: BATCHED UPSERT CONTACTS
-- Called by load-and-update-contacts.ts as: upsert_contacts_batch(p_offset, p_limit)
-- Processes a slice of import_contacts_staging (OFFSET / LIMIT).
-- Match priority: email → zoho_record_id → name+phone
-- ============================================================================
CREATE OR REPLACE FUNCTION upsert_contacts_batch(p_offset int, p_limit int)
...
-- ============================================================================
-- PART 4: DEDUPLICATE CONTACTS
-- Merges duplicates by email (case-insensitive) and name+phone.
-- Keeps the most recently updated record, merges data from older ones.
-- ============================================================================
...
-- ============================================================================
-- PART 5: BATCHED IMPORT NOTES
-- Called by load-and-update-contacts.ts as: import_notes_batch(p_offset, p_limit)
-- Links notes to parent contacts via zoho_record_id in data column.
-- Derives org_id from the parent record (avoids multi-org LIMIT 1 pitfall).
-- ============================================================================
CREATE OR REPLACE FUNCTION import_notes_batch(p_offset int, p_limit int)
...
BEGIN
  ...
  LOOP
    BEGIN
      -- Look up parent CRM record by Zoho Record ID (no org filter)
...
-- Notify PostgREST to pick up the new tables and functions
