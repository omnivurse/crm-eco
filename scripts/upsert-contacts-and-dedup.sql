-- ============================================================================
-- UPSERT CONTACTS FROM STAGING + DEDUPLICATE
-- 
-- This script:
--   1. Creates upsert_contacts_from_staging() - updates existing contacts
--      by email/record_id match, inserts new ones
--   2. Creates deduplicate_contacts() - removes duplicate contacts keeping
--      the most recently modified record
--
-- Prerequisites:
--   - Helper functions (_parse_import_date, etc.) from import-zoho-full.sql
--   - import_contacts_staging table populated with CSV data
--   - crm_records, crm_modules, organizations tables exist
--
-- Usage (Supabase SQL Editor):
--   1. Run this script to create the functions
--   2. Load CSV into import_contacts_staging (via load-csvs-to-staging.ts)
--   3. SELECT * FROM upsert_contacts_from_staging();
--   4. SELECT * FROM deduplicate_contacts();
-- ============================================================================


-- ============================================================================
-- FUNCTION 1: UPSERT CONTACTS FROM STAGING
-- 
-- Match logic (in order of priority):
--   1. Match by email (exact, case-insensitive)
--   2. Match by zoho_record_id in data->'zoho_record_id'
--   3. Match by first_name + last_name + phone (fallback for contacts without email)
-- 
-- If matched → UPDATE all fields (merge new data into existing)
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- ================================================================
      -- MATCH PRIORITY 2: By Zoho Record Id stored in data
      -- ================================================================
      ...
      -- ================================================================
      -- MATCH PRIORITY 3: By first_name + last_name + phone (fallback)
      -- ================================================================
      ...
      -- ================================================================
      -- Build comprehensive JSONB data object (same as original import)
      -- ================================================================
      ...
      -- ================================================================
      IF v_existing_id IS NOT NULL THEN
        -- UPDATE existing record: merge new data into existing data
        UPDATE crm_records SET
          ...
          -- Merge: existing data as base, overlay with new CSV data
          -- New non-null values overwrite, existing values preserved if CSV is empty
          data = (
            SELECT COALESCE(jsonb_object_agg(key, value), '{}'::jsonb)
...
-- ============================================================================
-- FUNCTION 2: DEDUPLICATE CONTACTS
-- 
-- Finds duplicate contacts by email (case-insensitive).
-- For each group of duplicates:
--   - Keeps the record with the most recent updated_at
--   - Merges data from older records into the keeper (so no data is lost)
--   - Deletes the older duplicate records
-- 
-- Also handles contacts with same first_name + last_name + phone as duplicates
-- ============================================================================
...
BEGIN
  ...
  -- ================================================================
  -- PHASE 1: Deduplicate by email (case-insensitive)
  -- ================================================================
  RAISE NOTICE '--- Phase 1: Deduplicating by email ---';
  ...
  LOOP
    ...
    -- The keeper is the most recently updated record
    ...
    -- Merge data from duplicates into keeper, then delete duplicates
    FOR v_dupe_rec IN
      SELECT id, data
      FROM crm_records
      WHERE org_id = v_org_id
      ...
      ORDER BY updated_at DESC
    LOOP
      -- Merge: keeper's data takes priority, fill in gaps from duplicate
      UPDATE crm_records SET
        data = (
          SELECT COALESCE(jsonb_object_agg(key, value), '{}'::jsonb)
          FROM jsonb_each(
            -- Duplicate data as base, overlay keeper's data on top
  ...
  RAISE NOTICE 'Phase 1 complete: % duplicate groups, % records merged, % records deleted',
    v_duplicates_found, v_records_merged, v_records_deleted;

  -- ================================================================
  -- PHASE 2: Deduplicate by first_name + last_name + phone
  -- (for contacts without email or with different emails)
  -- ================================================================
  RAISE NOTICE '--- Phase 2: Deduplicating by name + phone ---';

  FOR v_dup IN
    SELECT 
      LOWER(data->>'first_name') AS norm_first,
  ...
  RAISE NOTICE 'Phase 2 complete: % duplicate groups, % records deleted',
    v_np_duplicates_found, v_np_deleted;

  RAISE NOTICE 'Total deduplication: % email dups removed, % name+phone dups removed',
    v_records_deleted, v_np_deleted;
...
-- ============================================================================
-- QUICK DIAGNOSTIC QUERIES (run these to check before/after)
-- ============================================================================

-- Check how many contacts exist currently:
-- SELECT COUNT(*) AS total_contacts FROM crm_records 
...
-- SELECT COUNT(*) FROM import_contacts_staging WHERE record_id IS NOT NULL AND record_id != 'Record Id';

-- ============================================================================
-- EXECUTION ORDER:
--   1. Truncate staging:  TRUNCATE import_contacts_staging;
--   2. Load CSV via:      npx tsx scripts/load-csvs-to-staging.ts  (or load just contacts)
--   3. Upsert:            SELECT * FROM upsert_contacts_from_staging();
--   4. Dedup:             SELECT * FROM deduplicate_contacts();
-- ============================================================================
