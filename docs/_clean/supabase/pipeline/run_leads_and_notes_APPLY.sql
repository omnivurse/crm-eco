-- ============================================================================
-- APPLY: leads + notes only (NO contacts)  —  COMMITS at the end.
--
-- ⚠️  Only run this AFTER running run_leads_and_notes_DRYRUN.sql and
--     reviewing the deltas. This script is identical to the dry-run except
--     for COMMIT at the end instead of ROLLBACK.
--
-- Run with:
--   cd '/Users/qloudagent/Desktop/CRM DATA/_clean/supabase/pipeline'
--   psql "$SUPABASE_DB_URL" -f run_leads_and_notes_APPLY.sql 2>&1 | tee apply_run_$(date +%Y%m%d_%H%M%S).log
--
-- Safety guarantees:
--   * Contacts staging table and upsert function are NEVER touched
--   * \set ON_ERROR_STOP on  →  any error aborts the whole transaction
--   * The new upsert_leads_batch uses existing-wins JSONB merge, so even
--     if dedup matches an existing record (none expected today), staff
--     edits cannot be overwritten
--   * recover_zoho_notes_for_org is idempotent (±1min body-match dedup) —
--     re-running cannot create duplicate notes
-- ============================================================================
...
\echo ''
\echo '============================================================================'
\echo '  APPLY COMPLETE — transaction committed.'
\echo '  Review the deltas above. Leads should match staged row count, contacts'
\echo '  delta MUST be 0, notes delta = recover_zoho_notes_for_org.inserted_count.'
\echo '============================================================================'
