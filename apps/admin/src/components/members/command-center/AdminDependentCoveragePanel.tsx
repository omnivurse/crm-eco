...
/**
 * DependentCoverageHistory — Timeline of family member add/remove periods
 * for a linked membership (member_id). Staff can manage coverage via CRM
 * server actions when readOnly is false (default).
 */
...
import type { DependentCoveragePeriod } from '@crm-eco/lib';
import {
...
export const AdminDependentCoveragePanel = memo(function AdminDependentCoveragePanel({
  memberId,
  className,
  readOnly = false,
}: DependentCoverageHistoryProps) {
  ...
  const handleCoverageActionSave = async () => {
    ...
    try {
      let result;
      if (mode === 'end') {
        result = await adminEndDependentCoverage({
          member_id: memberId,
        ...
        result = await adminStartDependentCoverage({
          member_id: memberId,
          dependent_id: dependent.id,
        ...
        result = await adminLogHistoricalCoveragePeriod({
          member_id: memberId,
          dependent_id: dependent.id,
          effective_from: coverageForm.date,
  ...
  const handleAddDependent = async () => {
    ...
    setSaving(true);
    try {
      const result = await adminAddDependentWithCoverage({
        member_id: memberId,
