      ...
      <TableCell>
        ...
        >
          {isDoc ? (
            ...
            <Folder weight="light" className="w-4 h-4 shrink-0 text-blue-400" />
          )}
          <span className="truncate">{item.name}</span>
          {item.is_favorite && <Star weight="light" className="w-3 h-3 text-yellow-500 fill-yellow-500 shrink-0" />}
        </button>
      </TableCell>
          ...
          <DropdownMenuContent align="end">
            ...
            {isDoc && onVersions && (
              <DropdownMenuItem onClick={() => onVersions(item.id)}>
                <ClockCounterClockwise weight="light" className="w-4 h-4 mr-2" /> Version ClockCounterClockwise
              </DropdownMenuItem>
            )}
            <DropdownMenuSeparator />
