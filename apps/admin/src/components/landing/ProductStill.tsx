...
/**
 * A real screenshot of the real MMS console, server-rendered from the real
 * components with invented demo data. Two files ship — one per theme — and CSS
 * paints exactly one, because the theme is a class on <html> rather than an OS
 * preference, so <picture> + prefers-color-scheme cannot follow the nav toggle.
 * The unpainted one is `display: none`, so assistive tech only ever announces a
 * single image.
 *
...
export interface ProductStillProps {
  /** Base name under /public/landing, e.g. 'mms-queue' -> mms-queue-{theme}.png */
  name: string;
