...
/**
 * Vercel Cron route handler for commission processing.
 * Runs daily at 3:30 AM UTC; internally gated by billing_automation_config.
 */
export async function GET(request: Request) {
  try {
    ...
    // Fetch all orgs with commissions enabled
    ...
    if (configError) {
      console.error('Failed to fetch commission configs:', configError);
      return NextResponse.json(
        { error: 'Failed to fetch commission configs', details: configError.message },
        { status: 500 }
      );
    }
    ...
    if (!configs || configs.length === 0) {
      return NextResponse.json({
        message: 'No orgs with commissions enabled',
        processed: 0,
      });
    }
    ...
    return NextResponse.json({
      message: 'Commission cron completed',
      summary: { total: configs.length, processed, skipped, errored },
      results,
    });
  } catch (err) {
    console.error('Commission cron fatal error:', err);
    return NextResponse.json(
