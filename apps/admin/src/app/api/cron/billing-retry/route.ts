import { NextResponse } from 'next/server';
import { createServiceRoleClient } from '@crm-eco/lib/supabase/server';

export const dynamic = 'force-dynamic';
export const maxDuration = 300;

/**
 * Vercel Cron: dedicated retry processor for billing_failures that
 * weren't resolved in the main process-billing run.
 *
 * Runs daily at 16:00 UTC.
 */
export async function GET(request: Request) {
  try {
    const authHeader = request.headers.get('authorization');
    const expectedToken = process.env.CRON_SECRET;
    if (!expectedToken || authHeader !== `Bearer ${expectedToken}`) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const supabase = createServiceRoleClient();

    const { data: configs, error: configError } = await supabase
      .from('billing_automation_config')
      .select('organization_id')
      .eq('billing_enabled', true);

    if (configError) {
      return NextResponse.json(
        { error: 'Failed to fetch billing configs', details: configError.message },
        { status: 500 }
      );
    }

    if (!configs || configs.length === 0) {
      return NextResponse.json({ message: 'No orgs with billing enabled', processed: 0 });
    }

    const results: Array<{
      organization_id: string;
      status: 'success' | 'error';
      processed?: number;
      error?: string;
    }> = [];

    for (const config of configs) {
      try {
        const { data, error } = await supabase.functions.invoke('billing-retry', {
          body: { organization_id: config.organization_id },
        });

        if (error) {
          results.push({
            organization_id: config.organization_id,
            status: 'error',
            error: error.message,
          });
          await supabase.from('billing_job_runs').insert({
            organization_id: config.organization_id,
            job_type: 'billing_retry',
            triggered_by: 'cron',
            status: 'failed',
            error_log: [error.message],
          });
        } else {
          results.push({
            organization_id: config.organization_id,
            status: 'success',
            processed: data?.processed ?? 0,
          });
          await supabase.from('billing_job_runs').insert({
            organization_id: config.organization_id,
            job_type: 'billing_retry',
            triggered_by: 'cron',
            status: 'completed',
            metadata: data,
          });
        }
      } catch (err) {
        const message = err instanceof Error ? err.message : 'Unknown error';
        results.push({ organization_id: config.organization_id, status: 'error', error: message });
        await supabase.from('billing_job_runs').insert({
          organization_id: config.organization_id,
          job_type: 'billing_retry',
          triggered_by: 'cron',
          status: 'failed',
          error_log: [message],
        });
      }
    }

    return NextResponse.json({
      message: 'Billing retry cron completed',
      summary: {
        total: configs.length,
        processed: results.filter((r) => r.status === 'success').length,
        errored: results.filter((r) => r.status === 'error').length,
      },
      results,
    });
  } catch (err) {
    return NextResponse.json(
      { error: 'Internal server error', details: err instanceof Error ? err.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
