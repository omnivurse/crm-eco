...
import { ArrowLeft, Edit, Mail, Phone, MapPin, Calendar, Users, Palette } from 'lucide-react';
...
export default async function AgentDetailPage({ params }: { params: { id: string } }) {
  ...
  const [members, downline] = await Promise.all([
    getAgentMembers(params.id),
    getDownlineAgents(params.id),
  ]);
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{agent.commission_tier || '—'}</p>
              </div>
              <div>
                <p className="text-sm text-slate-500">Commission Eligible</p>
                <p className="font-medium">{agent.commission_eligible ? 'Yes' : 'No'}</p>
              </div>
            </CardContent>
          </Card>
