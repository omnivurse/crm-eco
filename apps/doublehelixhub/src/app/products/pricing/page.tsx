    ...
    <main className="min-h-screen bg-[#0a0f1a] text-white">
      <div className="mx-auto max-w-6xl px-6 py-16 space-y-10">
        <div className="space-y-3">
          ...
          <p className="max-w-2xl text-white/60 text-sm leading-relaxed">
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <p className="text-xs text-white/50 font-mono">{plan.planId}</p>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr>
                        <th className="border border-white/10 p-2 text-left text-white/70">
                          Coverage
                        </th>
                        {preview.ageBands.map((band) => (
                          <th
                            key={band.id}
                            className="border border-white/10 p-2 text-center text-white/70"
                          >
                            {band.label}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {preview.coverageTiers.map((tier) => (
                        <tr key={tier}>
                          <td className="border border-white/10 p-2 text-white/90">
                            {TIER_LABELS[tier]}
                          </td>
                          {preview.ageBands.map((band) => (
                            <td
                              key={band.id}
                              className="border border-white/10 p-2 text-center tabular-nums"
                            >
