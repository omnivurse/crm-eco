...
export default async function RxPage() {
  const providers = await listServiceProvidersByCategory('rx');
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      <PageHeader
        title="Prescription savings"
        description="Members save up to 80% on common prescriptions."
        kicker="Services"
        backHref="/services"
        backLabel="Back to Services"
        actions={
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[rgba(11,109,133,0.08)] text-[var(--mp-teal)]">
            <Pill weight="light" className="h-6 w-6" aria-hidden />
          </span>
        }
      />
