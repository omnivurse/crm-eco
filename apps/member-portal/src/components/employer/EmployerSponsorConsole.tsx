...
export function EmployerSponsorConsole({
  ...
  const canManageRoster = role === 'admin' || role === 'roster';
  const canManageApprovals = role === 'admin';
            ...
            <CardContent className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) =>
                    setEnrollMode(e.target.checked ? 'known_roster' : 'eligible_only')
                  }
                />
              ...
              {approvals.map((row) => (
                <li key={row.id} className="flex items-center justify-between gap-2 text-sm">
                  ...
                  {canManageApprovals ? (
                    ...
                    <span className="text-xs text-slate-500">Admin review required</span>
                  )}
                </li>
              ))}
            </ul>
