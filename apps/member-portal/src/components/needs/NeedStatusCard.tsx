...
import { FileText, Calendar, Buildings, WarningCircle, Target } from '@phosphor-icons/react/dist/ssr';
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-xl">
          <FileText weight="light" className="w-6 h-6 text-[var(--mp-teal)]" aria-hidden />
          Need Status
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-6">
          ...
          {!isTerminal && targetDate && (
            <div>
              <p className="text-sm text-slate-500 mb-1 flex items-center gap-1">
                <Target weight="light" className="w-3 h-3" aria-hidden /> Target Date
              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <p className="text-sm text-slate-500 mb-1 flex items-center gap-1">
              <FileText weight="light" className="w-4 h-4" aria-hidden /> Need Type
            </p>
          ...
          {need.incident_date && (
            <div>
              <p className="text-sm text-slate-500 mb-1 flex items-center gap-1">
                <Calendar weight="light" className="w-4 h-4" aria-hidden /> Service Date
              </p>
          ...
          {need.facility_name && (
            <div>
              <p className="text-sm text-slate-500 mb-1 flex items-center gap-1">
                <Buildings weight="light" className="w-4 h-4" aria-hidden /> Facility / Provider
              </p>
        ...
        {(need.status === 'awaiting_member_docs' || (!isTerminal && need.urgency_light === 'red')) && (
          <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <WarningCircle weight="light" className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" aria-hidden />
            <div>
