...
export function FiltersPanel({ fields, moduleKey, isOpen, onToggle }: FiltersPanelProps) {
  ...
  if (!isOpen) {
    return (
      <Button
        ...
        size="icon"
        onClick={onToggle}
        className="hidden lg:flex fixed left-14 top-20 z-20 h-9 w-9 rounded-lg bg-white dark:bg-slate-900 shadow-md border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
        title="Show filters"
      >
  ...
  return (
    <>
      ...
      {/* Filters Panel - Mobile: slide-in from left, Desktop: static sidebar */}
      <div className={cn(
        // Base styles
        'glass-strong border-r border-slate-200 dark:border-white/5 flex flex-col',
        // Mobile: fixed full-height slide-in
        'fixed lg:relative top-0 left-0 h-full z-50 lg:z-auto',
        'w-[85vw] max-w-[320px] lg:w-72 lg:max-w-none',
        // Mobile animation
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-white/5">
          ...
          <Button
            ...
            size="icon"
            onClick={onToggle}
            className="h-8 w-8 lg:h-7 lg:w-7 rounded-md text-slate-400 hover:text-slate-900 dark:hover:text-white"
          >
          ...
          <form onSubmit={handleSearch}>
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 h-10 lg:h-9 text-sm bg-white dark:bg-slate-900/50 border-slate-200 dark:border-white/10"
              />
            </div>
          </form>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
              ...
              <div key={index} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className="h-7 w-7 lg:h-6 lg:w-6 text-slate-400 hover:text-red-500"
                  >
                    <X className="w-3 h-3" />
                  </Button>
                </div>
          ...
          <Button
            ...
            size="sm"
            onClick={addFilter}
            className="w-full h-10 lg:h-8 text-sm text-teal-600 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-500/10"
          >
        ...
        {/* Actions */}
        <div className="p-4 border-t border-slate-200 dark:border-white/5 space-y-2 pb-[max(1rem,env(safe-area-inset-bottom))]">
          <Button
            onClick={() => {
              applyFilters();
          ...
          {(filters.length > 0 || searchQuery) && (
            <Button
              ...
              className="w-full text-slate-600 dark:text-slate-400 h-11 lg:h-10"
            >
              Clear All
            </Button>
          )}
