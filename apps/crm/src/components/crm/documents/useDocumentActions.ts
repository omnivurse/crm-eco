...
export function useDocumentActions() {
  ...
  const downloadFile = useCallback(async (documentId: string, versionNumber?: number) => {
    const result = await invokeEdgeFunction('doc-download', {
      document_id: documentId,
  ...
  const toggleFavorite = useCallback(async (
    ...
    if (isFavorite) {
      ...
      if (type === 'document') {
        await query.eq('document_id', id).eq('user_id', user.id);
      } else {
        await query.eq('folder_id', id).eq('user_id', user.id);
      }
  ...
  const moveItems = useCallback(async (docIds: string[], folderIds: string[], targetFolderId: string | null) => {
    return invokeEdgeFunction('doc-bulk', {
      action: 'move',
      ids: docIds,
  ...
  const createShareLink = useCallback(async (
    ...
    await supabase.from('document_audit_log').insert({
      ...
      user_id: user.id,
      action: 'share',
      meta: { expires_in_hours: expiresInHours, max_downloads: maxDownloads },
    });
