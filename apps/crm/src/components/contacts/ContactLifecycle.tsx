...
export default function ContactLifecycle() {
  ...
  const { summary, monthlyTrends, cancellationReasons, recentEvents } = data;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Lifecycle</h2>
          <p className="text-slate-600 dark:text-slate-400">
            Member enrollment, cancellation, and retention tracking
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card className="glass-card border-slate-200 dark:border-white/10">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white">
              {summary.totalTrackedMembers.toLocaleString()}
            </p>
        ...
        <Card className="glass-card border-slate-200 dark:border-white/10">
          <CardContent className="pt-6">
            ...
            <p className="text-3xl font-bold text-slate-900 dark:text-white">
              {summary.churnRate12m}%
            </p>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">12-Month Churn</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="glass-card border-slate-200 dark:border-white/10 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-slate-900 dark:text-white">Monthly Timeline</CardTitle>
            <CardDescription>Enrollment and cancellation events over time</CardDescription>
          </CardHeader>
          <CardContent>
            {monthlyTrends.length === 0 ? (
              ...
              <div className="space-y-2">
                <div className="flex gap-4 text-xs mb-4">
                  ...
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded bg-blue-500" /> Returned
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded bg-amber-500" /> Paused
                  </span>
                </div>
                {monthlyTrends.slice(-12).map((m) => {
                  const maxVal = Math.max(m.enrolled, m.cancelled, m.returned, m.paused, 1);
                  return (
                    <div key={m.month} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 flex gap-1 items-center">
                        ...
                        />
                        <div
                          className="h-5 bg-blue-500/80 rounded"
                          style={{ width: `${(m.returned / maxVal) * 100}%`, minWidth: m.returned > 0 ? 4 : 0 }}
                        />
                        <div
                          className="h-5 bg-amber-500/80 rounded"
                          style={{ width: `${(m.paused / maxVal) * 100}%`, minWidth: m.paused > 0 ? 4 : 0 }}
                        />
                      </div>
        ...
        <Card className="glass-card border-slate-200 dark:border-white/10">
          <CardHeader>
            ...
            <CardDescription>
              {cancellationReasons.reduce((s, r) => s + r.count, 0)} total cancellations
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div key={r.reason} className="space-y-1">
                    ...
                    <div className="h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(r.count / (cancellationReasons[0]?.count || 1)) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
      ...
      <Card className="glass-card border-slate-200 dark:border-white/10">
        <CardHeader>
          ...
          <CardDescription>Latest lifecycle changes across all contacts</CardDescription>
        </CardHeader>
        <CardContent>
          {recentEvents.length === 0 ? (
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {recentEvents.map((e) => (
                <div
                  key={e.id}
                  className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-colors"
                >
                  <div
                    className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 ${EVENT_COLORS[e.event_type] || 'bg-slate-400'}`}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-900 dark:text-white truncate">
                      {e.contact?.title || e.contact?.email || 'Unknown Contact'}{' '}
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                      {e.event_date}
                      {e.reason ? ` -- ${REASON_LABELS[e.reason] || e.reason}` : ''}
                      {e.plan_type ? ` -- ${e.plan_type}` : ''}
                      {e.advisor
                        ? ` -- Advisor: ${e.advisor.first_name} ${e.advisor.last_name}`
                        : ''}
                    </p>
                  </div>
                </div>
