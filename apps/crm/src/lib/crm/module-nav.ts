/**
 * Server-safe CRM nav catalog. Keep this file free of `use client` / next/navigation.
 * `/api/gizmo/chat` imports palette-pages, which must not pull ModuleContext
 * (Next wraps client-module value exports; TOP_MODULES.map then dies at collect).
 */

import { PEOPLE_SECTION_LABEL } from './nav-lexicon';
