      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="glass-card rounded-xl p-4 border border-slate-200 dark:border-white/10 animate-pulse"
            >
        ...
        <div className="space-y-3">
          {filteredReports.map((report) => (
            <div
              key={report.id}
              className="glass-card rounded-xl p-4 border border-slate-200 dark:border-white/10 hover:border-teal-500/20 transition-all"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <Link
                      href={`/crm/reports/saved/${report.id}`}
                      className="text-lg font-semibold text-slate-900 dark:text-white hover:text-teal-600 dark:hover:text-teal-400 transition-colors truncate"
                    >
                      {report.name}
                    </Link>
                    {report.is_favorite && (
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem asChild>
                        <Link href={`/crm/reports/saved/${report.id}`}>
