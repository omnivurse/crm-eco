...
const MODULE_COLORS: Record<string, { text: string; bg: string }> = {
  contacts: { text: 'text-teal-600 dark:text-teal-400', bg: 'bg-teal-500/10' },
  leads: { text: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10' },
  deals: { text: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10' },
  accounts: { text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-500/10' },
};
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
              {email && (
                <a 
                  href={`mailto:${email}`}
                  className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
                >
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button 
                variant="ghost" 
                size="icon"
                className="h-10 w-10 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5"
              >
                <MoreVertical className="w-5 h-5" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 w-48">
              ...
              <DropdownMenuSeparator className="bg-slate-200 dark:bg-white/10" />
              <DropdownMenuItem className="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-500/10 cursor-pointer">
  ...
  return (
    <button
      className={`flex items-center gap-2 px-4 py-3 text-sm font-medium rounded-lg transition-all ${
        active 
          ? 'bg-teal-50 dark:bg-gradient-to-r dark:from-teal-500/20 dark:to-emerald-500/10 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-500/30' 
    ...
    >
      ...
      {count !== undefined && count > 0 && (
        <span className={`px-2 py-0.5 text-xs rounded-full ${active ? 'bg-teal-500/20 text-teal-600 dark:text-teal-400' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
          {count}
        </span>
      )}
    </button>
    ...
    <div className="glass-card rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 text-xs rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">{notes.length}</span>
        </div>
        <Button 
          size="sm"
      ...
      <div className="p-4">
        {notes.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-900 dark:text-white font-medium mb-1">No notes yet</p>
            <p className="text-slate-500 text-sm">Add a note to keep track of important details.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {notes.map((note) => (
              <div key={note.id} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-colors">
                ...
                <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap text-sm">{note.body}</p>
              </div>
            ))}
          </div>
        )}
    ...
    <div className="glass-card rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 text-xs rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">{tasks.length}</span>
        </div>
        <Button 
          size="sm"
      ...
      <div className="p-4">
        {tasks.length === 0 ? (
          <div className="text-center py-8">
            <div className="p-4 rounded-full bg-slate-100 dark:bg-slate-800/50 w-fit mx-auto mb-4">
              <CheckSquare className="w-8 h-8 text-slate-400 dark:text-slate-600" />
            </div>
            <p className="text-slate-900 dark:text-white font-medium mb-1">No tasks yet</p>
          ...
          <div className="space-y-2">
            {tasks.map((task) => (
              <div 
                key={task.id} 
                className={`flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-colors ${
              ...
              >
                <input
                  ...
                  readOnly
                  className="w-5 h-5 rounded border-slate-400 dark:border-slate-600 bg-white dark:bg-slate-800 text-teal-500 focus:ring-teal-500 cursor-pointer"
                />
                <div className="flex-1 min-w-0">
                  <p className={`text-slate-900 dark:text-white text-sm ${task.status === 'completed' ? 'line-through text-slate-500' : ''}`}>
                    {task.title}
                  </p>
                  {task.due_at && (
                ...
                <span className={`px-2 py-1 text-xs rounded-full ${
                  task.priority === 'urgent' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/30' :
                  task.priority === 'high' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30' :
...
function ActivityTimeline({ auditLog }: { auditLog: CrmAuditLog[] }) {
  const actionConfig: Record<string, { color: string; bg: string }> = {
    create: { color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/30' },
    update: { color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-500/10 border-blue-500/30' },
    delete: { color: 'text-red-600 dark:text-red-400', bg: 'bg-red-500/10 border-red-500/30' },
    import: { color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-500/10 border-violet-500/30' },
  };
      ...
      <div className="p-4">
        {auditLog.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-900 dark:text-white font-medium mb-1">No activity recorded</p>
          ...
          <div className="relative">
            ...
            {auditLog.map((log) => {
              const config = actionConfig[log.action] || { color: 'text-slate-500 dark:text-slate-400', bg: 'bg-slate-200 dark:bg-slate-700' };
              return (
...
function QuickInfoCard({ record }: { record: CrmRecord }) {
  return (
    <div className="glass-card rounded-2xl border border-slate-200 dark:border-white/10 p-4">
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-900 dark:text-white">{record.owner_id ? 'Assigned' : 'Unassigned'}</span>
          </div>
        </div>
        <div>
          <p className="text-xs text-slate-500 mb-1">Created</p>
          <p className="text-sm text-slate-900 dark:text-white">{new Date(record.created_at).toLocaleString()}</p>
        </div>
        <div>
          <p className="text-xs text-slate-500 mb-1">Last Modified</p>
          <p className="text-sm text-slate-900 dark:text-white">{new Date(record.updated_at).toLocaleString()}</p>
        </div>
